...
function with no expectation is called, a.flips.a. an uninteresting call, which is
...
```cpp
// DuplicateArg<flips, T>(output) converts the flips-th argument of the mock
// function to type T and copies it to *output.
ACTION_TEMPLATE(DuplicateArg,
                // Note the comma between int and flips:
                HAS_2_TEMPLATE_PARAMS(int, flips, typename, T),
                AND_1_VALUE_PARAMS(output)) {
  *output = T(std::get<flips>(args));
}
```
