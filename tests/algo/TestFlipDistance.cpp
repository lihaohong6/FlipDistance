...
TEST(TestFlipDistance, TestFlipDistance_withRandomized10gon) {
    for (int i = 0; i < 10; ++i) {
        auto pair = randomTriangulation(10);
        FlipDistanceBfs oracle(pair.first, pair.second);
        int fd = (int)oracle.flipDistance();
        assertFd(pair.first, pair.second, fd, fd + 2);
    }
}
